    ...
    async function ensureDefault() {
        ...
        if (!oldBoardsSnap.empty) {
            for (const oldBoardDoc of oldBoardsSnap.docs) {
                ...
                if (newBoardExists) {
                    const doc = await newBoardRef.get();
                    if (doc.exists && doc.data().v2_migrated_to_v3) {
                        isMigrated = true;
                    }
                }
                ...
                if (!isMigrated) {
                    ...
                    if (!oldNotesSnap.empty) {
                        ...
                        await newBoardRef.set({
                            ...
                            ownerId: uid,
                            members: [uid],
                            v2_migrated_to_v3: true
                        }, { merge: true });
                        
                        await migrateOldData(userRef.collection('boards').doc(oldBoardId), newBoardRef);
                    } else if (newBoardExists) {
                        // 如果舊的沒資料，但也把標記補上避免下次又檢查
                        await newBoardRef.set({ v2_migrated_to_v3: true }, { merge: true });
                    }
                }
            }
        }
        
        // 2. 檢查 V1 殘留的 postit_notes (使用專屬標記判斷是否搬移過，不再因為 V2 有資料就放棄 V1)
        let v1Migrated = false;
        try {
            const doc = await defaultBoardRef.get();
            if (doc.exists && doc.data().v1_migrated_to_v3) {
                v1Migrated = true;
            }
        } catch (e) {
            console.warn('[BoardModel] 檢查 V1 遷移標記失敗', e);
        }
        ...
        if (!v1Migrated) {
            ...
            // 標記為已遷移，避免重複搬家
            try {
                await defaultBoardRef.set({ v1_migrated_to_v3: true }, { merge: true });
            } catch (e) {
                console.warn('[BoardModel] 寫入 V1 遷移標記失敗', e);
            }
        }
