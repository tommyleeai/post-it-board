...
10. **最重要**：若使用者的意圖是「待辦事項提取」（hasTodoItems 為 true），且文字中沒有明確提及未來的具體時間點或日期（例如僅有「我要買香蕉，蘋果...」而沒有「明天」、「下午五點」等明確時間），則時間提醒部分應設為 hasIntent: false、needsClarification: false。因為這是一張單純的待辦清單卡片，不需要反問使用者任何時間問題。
...
- 若不是待辦清單，hasTodoItems 設為 false，todoTitle 設為空字串，todoItems 設為空陣列 []`;
    ...
    async function parseIntent(noteText, otherNotesContext = '') {
        ...
        const engine = (typeof PostIt.Settings.getAiEngine === 'function') ? PostIt.Settings.getAiEngine() : 'gemini';
        if (engine === 'minimax') {
            return await parseIntentMinimax(noteText, otherNotesContext);
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (typeof PostIt.Board !== 'undefined') {
                    let errMsg = `AI 解析失敗 (HTTP ${response.status})`;
                    const isDunning = errText.includes('dunning') || errText.includes('billing');
                    const isInvalidKey = errText.includes('expired') || errText.includes('API_KEY_INVALID') || response.status === 400;

                    if (isDunning) {
                        errMsg = 'AI 解析失敗：您的 Google Cloud 專案似乎有帳單欠費或停用問題 (Dunning)，請檢查 GCP 帳單或更換 API Key。';
                    } else if (isInvalidKey) {
                        errMsg = 'AI 解析失敗：API Key 已過期或無效，請檢查設定。';
                    } else if (response.status === 429) {
                        errMsg = 'AI 解析失敗：呼叫次數已達上限 (429)。';
                    } else if (response.status === 503) {
                        errMsg = 'AI 解析失敗：AI 伺服器目前大塞車 (503)。';
                    } else {
                        errMsg = 'AI 解析拒絕 (HTTP ' + response.status + ')：' + errText.substring(0, 100);
                    }
                    ...
                    if (ollamaSettings && ollamaSettings.enableFallback && ollamaSettings.url && ollamaSettings.model) {
                        PostIt.Board.showToast(`${errMsg} 正在切換至本地端 Ollama 備援解析...`, 'info', null, 6000);
                        return await fallbackToOllama(prompt, ollamaSettings);
                    } else {
                        PostIt.Board.showToast(errMsg, 'error', null, 8000);
                    }
                }
            ...
            // 強制防呆：如果是待辦清單且無明確時間，強制關閉時間提醒反問
            if (resultObj.hasTodoItems && resultObj.needsClarification) {
                resultObj.hasIntent = false;
                resultObj.needsClarification = false;
                resultObj.clarificationQuestion = '';
            }
    ...
    async function testGemini(apiKey) {
        ...
        try {
            ...
            if (!response.ok) {
                const errText = await response.text();
                if (errText.includes('dunning') || errText.includes('billing')) {
                    return { success: false, msg: '專案帳單欠費或停用 (Dunning)，請檢查 GCP 帳單設定' };
                } else if (response.status === 400 || errText.includes('API_KEY_INVALID') || errText.includes('API key not valid')) {
                ...
                return { success: false, msg: `伺服器回應錯誤 (HTTP ${response.status})：${errText.substring(0, 50)}` };
            }
    ...
    async function parseIntentMinimax(noteText, otherNotesContext = '') {
        if (!noteText || noteText.trim() === '') return { hasIntent: false };

        const apiKey = PostIt.Settings.getMinimaxKey();
        if (!apiKey) {
            console.warn('[AI MiniMax] 尚未設定 API Key');
            return { hasIntent: false, error: 'NO_API_KEY' };
        }
        ...
        try {
            ...
            if (!response.ok) {
                const errText = await response.text();
                console.error('[AI MiniMax] API 調用失敗:', errText);
                
                if (typeof PostIt.Board !== 'undefined') {
                    let errMsg = `AI 解析失敗 (HTTP ${response.status})`;
                    const isArrears = errText.includes('arrears') || errText.includes('balance') || errText.includes('insufficient') || response.status === 402;
                    const isInvalidKey = errText.includes('invalid') || errText.includes('auth') || response.status === 401;

                    if (isArrears) {
                        errMsg = 'AI 解析失敗：您的 MiniMax 帳戶餘額不足或已欠費，請檢查 MiniMax 帳單設定。';
                    } else if (isInvalidKey) {
                        errMsg = 'AI 解析失敗：MiniMax API Key 無效或已過期，請檢查設定。';
                    } else if (response.status === 429) {
                        errMsg = 'AI 解析失敗：MiniMax 呼叫次數已達上限 (429)。';
                    } else {
                        errMsg = 'AI 解析拒絕 (HTTP ' + response.status + ')：' + errText.substring(0, 100);
                    }
            ...
            console.error('[AI MiniMax] 語意解析發生不可預期錯誤:', error);
            if (typeof PostIt.Board !== 'undefined') {
                PostIt.Board.showToast('AI 解析發生錯誤：' + (error.message || '未知錯誤'), 'error', null, 8000);
            }
            return { hasIntent: false, error: 'PARSE_ERROR' };
    ...
    async function testMinimax(apiKey) {
        if (!apiKey || apiKey.trim() === '') return { success: false, msg: 'API Key 不可為空' };
        try {
            const url = 'https://api.minimaxi.com/v1/chat/completions';
            const response = await fetch(url, {
                ...
                body: JSON.stringify({
                    model: "MiniMax-M2.7",
                    messages: [{ role: "user", content: "這是一個網路傳輸與模型負載測試，請只回覆我一個字：OK" }]
                })
            });

            if (!response.ok) {
                const errText = await response.text();
                if (errText.includes('arrears') || errText.includes('balance') || errText.includes('insufficient') || response.status === 402) {
                    return { success: false, msg: 'MiniMax 帳戶餘額不足或欠費，請檢查帳單' };
                } else if (response.status === 401 || errText.includes('invalid') || errText.includes('auth')) {
                    return { success: false, msg: 'MiniMax API Key 無效或已過期' };
                } else if (response.status === 429) {
                    return { success: false, msg: '呼叫次數達上限 (429)' };
                }
                return { success: false, msg: `伺服器回應錯誤 (HTTP ${response.status})：${errText.substring(0, 50)}` };
            }
            return { success: true, msg: '✅ 雲端 MiniMax 連線測試成功！' };
        } catch (error) {
            return { success: false, msg: '無法連線至 MiniMax 伺服器，請檢查網路' };
        }
    }

    return { parseIntent, testOllama, testGemini, testMinimax };
})();
