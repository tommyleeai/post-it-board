...
2. 請依照傳入的「目前本地時間」推算。回傳時間務必是 ISO 8601 格式但「絕對不要帶 Z 或是 +08:00」。`;
    ...
    async function parseIntent(noteText) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (typeof PostIt.Board !== 'undefined') {
                    if (errText.includes('expired') || errText.includes('API_KEY_INVALID')) {
                        ...
                        PostIt.Board.showToast('AI 解析失敗：呼叫次數達上限 (429)，請稍等一分鐘', 'error', null, 6000);
                    } else if (response.status === 503) {
                        PostIt.Board.showToast('AI 伺服器目前大塞車 (Google 503 High Demand)，請稍等幾分鐘後再改字重試！', 'error', null, 6000);
                    } else {
                        // 未知錯誤，直接顯示大略內容
            ...
            // 防呆：如果 AI 偷懶沒給 alertTime 但有給 eventTime，我們自己手動幫它提早 10 分鐘
            if (resultObj.hasIntent && resultObj.eventTime && !resultObj.alertTime) {
                try {
                    const eTime = new Date(resultObj.eventTime);
                    eTime.setMinutes(eTime.getMinutes() - 10);
                    const y = eTime.getFullYear();
                    const m = String(eTime.getMonth() + 1).padStart(2, '0');
                    const d = String(eTime.getDate()).padStart(2, '0');
                    const h = String(eTime.getHours()).padStart(2, '0');
                    const min = String(eTime.getMinutes()).padStart(2, '0');
                    const s = String(eTime.getSeconds()).padStart(2, '0');
                    resultObj.alertTime = `${y}-${m}-${d}T${h}:${min}:${s}`;
                    console.log('[AI] 自動補齊假單的 alertTime:', resultObj.alertTime);
                } catch(e) {
                    console.error('[AI] 補齊 alertTime 失敗', e);
                }
            }
            ...
            // 臨時除錯：在 UI 顯示 AI 的解析結果
            if (typeof PostIt.Board !== 'undefined') {
                const debugStr = JSON.stringify(resultObj);
                PostIt.Board.showToast('AI 解析完畢: ' + debugStr, 'info', null, 8000);
            }
