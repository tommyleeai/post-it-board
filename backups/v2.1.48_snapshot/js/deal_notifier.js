...
PostIt.DealNotifier = (function () {
    ...
    async function triggerAlert(deal) {
        ...
        // 固定出現於畫面左下角 (大約 x: 5%, y: 75%)
        const newNoteId = await PostIt.Note.create(payloadStr, 'super_deal', '#FFFFFF', 'system', { x: 5, y: 75 });
        
        if (newNoteId) {
            // 自動尋找現有的超級好物便利貼，將新卡片加入其群組中
