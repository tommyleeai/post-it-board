...
PostIt.Group = (function () {
    ...
    function checkOverlap(dragEl) {
        ...
        // 橫式純圖片便利貼（寬度加倍）禁止群組合併
        if (dragEl.classList.contains('landscape-image') && dragEl.classList.contains('image-only')) return;

        document.querySelectorAll('.sticky-note').forEach(function (el) {
            if (el === dragEl) return;
            // 橫式純圖片便利貼禁止被合併
            if (el.classList.contains('landscape-image') && el.classList.contains('image-only')) return;
