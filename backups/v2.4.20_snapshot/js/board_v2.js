...
PostIt.Board = (function () {
    ...
    // ======== 統一圖片複製邏輯 ========
    // 直接選取畫面上已載入的 <img> 元素，用瀏覽器原生複製
    // 不需要 fetch/canvas/CORS，因為圖片已經在 DOM 中了
    function copyImageToClipboard(imageUrl) {
        ...
        // 先從燈箱找
        const lightboxImg = document.getElementById('lightbox-image');
        if (lightboxImg && lightboxImg.src && lightboxImg.src === imageUrl && isLightboxOpen()) {
            imgEl = lightboxImg;
        }

        // 從白板上找
        if (!imgEl) {
            const allImgs = document.querySelectorAll('img.note-img');
            for (const img of allImgs) {
                if (img.src === imageUrl) { imgEl = img; break; }
            }
        }

        if (!imgEl || !imgEl.complete || imgEl.naturalWidth === 0) {
            showToast('圖片尚未載入完成', 'error');
            return;
        }

        // 選取 img 元素，讓瀏覽器原生複製（含圖片資料）
        const range = document.createRange();
        ...
        try {
            ...
            console.error('[Copy] execCommand 失敗:', err);
            showToast('複製失敗', 'error');
        }
