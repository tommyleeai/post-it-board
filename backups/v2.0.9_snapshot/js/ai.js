        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (errText.includes('expired') || errText.includes('API_KEY_INVALID')) {
                    if (typeof PostIt.Board !== 'undefined') PostIt.Board.showToast('AI 解析失敗：API Key 已過期或無效', 'error');
                } else if (response.status === 429) {
                    if (typeof PostIt.Board !== 'undefined') PostIt.Board.showToast('AI 解析失敗：呼叫次數達上限', 'error');
                }
