...
PostIt.YjsSync = (function () {
    ...
    async function migrateOldNotesToYjs(boardId, isMigrated) {
        try {
            ...
            const docSnap = await db.collection('boards').doc(boardId).get();
            if (docSnap.exists && docSnap.data().v3_deep_migrated_v7) {
            ...
            // 1. 保留今天新建立的有效便利貼，清除所有格式錯誤的幽靈便利貼 (字串 px、過大座標、缺乏 content)
            ...
            for (const key of currentKeys) {
                ...
                if (createdAt && typeof createdAt.seconds === 'number' && createdAt.seconds > 1776900000) {
                    if (!isBrokenX && !isBrokenY && !isMissingContent) {
                        continue; // 座標正確且內容正常的今天新筆記，保留它
                    }
                }
                // 刪除壞掉的或太舊的，讓後面從 Firestore 重新載入正確版
            ...
            // 標記為已深度遷移
            await db.collection('boards').doc(boardId).set({ v3_deep_migrated_v7: true }, { merge: true });
        } catch (e) {
