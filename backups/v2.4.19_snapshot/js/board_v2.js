...
PostIt.Board = (function () {
    ...
    function copyImageToClipboard(imageUrl) {
        const blobPromise = (async () => {
            ...
            // 第一優先：用 XHR + responseType blob 下載（Firebase Storage 圖片適用）
            if (imageUrl.includes('firebasestorage.googleapis.com') || imageUrl.includes('firebasestorage.app')) {
                try {
                    ...
                    console.warn('[Copy] XHR 取圖失敗:', err);
                }
            }
            ...
            if (!rawBlob) {
                try {
                    ...
                    console.warn('[Copy] fetch 失敗:', err);
                }
            }
            ...
            if (rawBlob) {
                ...
                // 非 PNG 需轉成 PNG（Clipboard API 只接受 PNG）
            ...
            try {
                ...
                console.warn('[Copy] canvas with CORS 失敗:', err);
            }

            // 最後手段：不帶 crossOrigin 的 canvas
            return canvasToPngBlobNoCors(imageUrl);
        })();

        // clipboard.write 在使用者手勢的同步流程中呼叫
        navigator.clipboard.write([
            new ClipboardItem({ 'image/png': blobPromise })
        ]).then(() => {
            showToast('已複製', 'success', null, 1000);
        }).catch(err => {
            console.error('[Copy] 複製圖片最終失敗:', err);
