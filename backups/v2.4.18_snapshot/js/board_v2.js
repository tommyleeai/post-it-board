...
PostIt.Board = (function () {
    ...
    function bindUIEvents() {
        ...
        if (lightbox && btnCloseLightbox && lightboxImg && contextMenu) {
            ...
            lightboxImg.addEventListener('contextmenu', (e) => {
                e.preventDefault();
                e.stopPropagation();
                copyImageToClipboard(lightboxImg.src);
            });
        }
    }
    ...
    // ======== 統一圖片複製邏輯 ========
    // 關鍵：使用 ClipboardItem + Promise<Blob> 模式，確保 clipboard.write()
    // 在使用者手勢 (user gesture) 的同步上下文中執行，否則瀏覽器會拒絕寫入
    function copyImageToClipboard(imageUrl) {
        // 將 fetch + 轉換包裝成 Promise<Blob>，傳入 ClipboardItem
        const blobPromise = (async () => {
            // 第一優先：嘗試 fetch（Firebase Storage 有 CORS 支援）
            try {
                ...
                const pngBlob = await canvasToPngBlob(objUrl);
                URL.revokeObjectURL(objUrl);
                return pngBlob;
            } catch (err) {
                console.warn('fetch 複製失敗，嘗試 canvas fallback:', err);
            }
            // 第二優先：用 canvas 直接繪製（帶 crossOrigin）
            try {
                return await canvasToPngBlob(imageUrl);
            } catch (err) {
                console.warn('crossOrigin canvas 失敗，嘗試不帶 CORS:', err);
            }
            // 第三優先：不帶 crossOrigin（避免 CORS 問題，但可能觸發 tainted canvas）
            return canvasToPngBlobNoCors(imageUrl);
        })();

        // clipboard.write 必須在使用者手勢的同步流程中呼叫
        navigator.clipboard.write([
            new ClipboardItem({ 'image/png': blobPromise })
        ]).then(() => {
            showToast('已複製', 'success', null, 1000);
        }).catch(err => {
    ...
    function canvasToPngBlob(imageSrc) {
    ...
    // 不帶 crossOrigin 的 fallback（對同域圖片或已快取圖片有效）
    function canvasToPngBlobNoCors(imageSrc) {
        return new Promise((resolve, reject) => {
            const canvas = document.createElement('canvas');
            const ctx = canvas.getContext('2d');
            const tempImg = new Image();
