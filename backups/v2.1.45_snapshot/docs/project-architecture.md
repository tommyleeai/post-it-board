...
# 由於 PowerShell 執行原則，若直接執行 npm run dev 失敗，請改用：
# cmd /c "npx http-server ./ -p 5051 -a localhost -c-1 --cors -o"
...
*   **【強制規則】**：每次 AI 請使用者進行功能測試前，**必須先主動幫使用者把 localhost 伺服器打開**，以確保測試環境準備就緒。
