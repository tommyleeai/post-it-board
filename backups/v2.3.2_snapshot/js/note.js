...
PostIt.Note = (function () {
    ...
    // -------- 訂閱 Yjs 即時更新 --------
    async function subscribe(onUpdate) {
        cleanup();
            ...
            if (groupIdA && groupIdB && groupIdA !== groupIdB) {
                ...
                bMembers.forEach((member, i) => {
                    ...
                    if (yNote) {
                        ...
                        yNote.set('groupOrder', existingCount + i);
                ...
                const yNoteLone = yNotesMap.get(loneId);
                if (yNoteLone) {
                    yNoteLone.set('groupId', targetGroupId);
                    yNoteLone.set('groupOrder', existingCount);
                    yNoteLone.set('groupOffsetX', (Math.random() - 0.5) * 6);
                    yNoteLone.set('groupOffsetY', (Math.random() - 0.5) * 6);
                }
            }
